...
verbose = False
debug = False
data_path = '/home/opc/new_data_twitter/replies_to_month'
data_csv_path = '/home/opc/new_data_twitter_csv/replies_to_month'
...
def scrape_replies_by_user(us, since, until):
	...
	print('[scrape_replies_by_user] {} tweets | {} | user'.format(len(tweets_df.index), us))
	if len(tweets_df.index) > 0:
		df_csv = extendDataframe(tweets_df, '{}'.format(us))
		print('[scrape_replies_by_user] {} to tweets | {} | user'.format(len(df_csv.index), us))
		df_main = df_csv[(df_csv['conversation_id'] == df_csv['id'])]
		df_replies = df_csv[(df_csv['conversation_id'] != df_csv['id'])]
		print('[scrape_replies_by_user] {} main tweets | {} replies tweets | {} | user'.format(len(df_main.index),len(df_replies.index), us))
		# Save
...
def main():
    ...
    #data = process_yaml()
    # Execute Method
    scrape_replies_by_user(args.user, args.Since, args.Until)
    print("Done!")
    #
