...
def main():
    ...
    generateHashtagTerms(df_public)
    generateTweetsUsersTagged(df_public)
    generateMapTweetTopTerms(df_public)
    #analyzeTweetsSentiment(df_public)
    # Execute all Threads
    #t1 = Thread(target=generateHashtagTerms, args=(df_public,))
    #t2 = Thread(target=generateTweetsUsersTagged, args=(df_public,))
    #t3 = Thread(target=generateMapTweetTopTerms, args=(df_public,))
    #t4 = Thread(target=analyzeTweetsSentiment, args=(df_public,))
